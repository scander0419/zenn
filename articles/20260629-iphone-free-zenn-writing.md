...
前回、Codex で Zenn 記事を PR 化し、スマホでレビューして merge で同期する仕組みを作りました。

https://zenn.dev/kakecake/articles/codex-zenn-pr-sync-flow

ただ、この構成には穴がありました。スマホでできるのは「レビューして merge する」ところまでで、**記事を書く**のは結局 PC が必要でした。この記事はその続きで、出先の iPhone だけで記事を書いて PR まで出せるようにします。

手書きではなく Markdown をタイプして、最終的に PR 作成まで終わらせる構成です。ついでに「このPCでどう書いているか」も文章として残っていなかったので、PC用とiPhone用の執筆手順も整えました。その過程で、OneDrive 配下のリポジトリ特有の CRLF 問題を踏んだので、そこも含めて書きます。
...
前回作った「PR化してスマホでレビュー → merge で同期」の仕組みと合わせると、**書く → レビュー → 公開** までスマホだけで回せるようになります。
